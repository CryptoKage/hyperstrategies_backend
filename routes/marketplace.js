...
router.post('/tabs/list', async (req, res) => {
  ...
  if (!tabId || !price) {
    return res.status(400).json({ error: 'tabId and price required' });
  }
...
// Buy a tab listing
router.post('/tabs/:listingId/buy', async (req, res) => {
  ...
  try {
    ...
    await client.query('UPDATE users SET balance = balance - $1 WHERE user_id = $2', [listing.price, buyerId]);
    await client.query('UPDATE users SET balance = balance + $1 WHERE user_id = $2', [listing.price, listing.seller_id]);
