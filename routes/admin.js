...
router.post('/rewards/execute-hybrid-distribution', async (req, res) => {
    const { totalRewardUsd, participatingVaultIds, description: overallDescription } = req.body;
    const adminUserId = req.user.id;
    const numericReward = parseFloat(totalRewardUsd);

    if (isNaN(numericReward) || numericReward <= 0 || !Array.isArray(participatingVaultIds) || participatingVaultIds.length === 0 || !overallDescription) {
        return res.status(400).json({ error: 'A valid totalRewardUsd, participatingVaultIds, and a description are required.' });
    }
    ...
    try {
        ...
        for (const user of eligibleUsers) {
            ...
            if (userBuybackAmount <= 0.000001 && userXpAmount <= 0.000001) continue;
            ...
            if (userBuybackAmount > 0.000001) {
                await client.query(`INSERT INTO vault_ledger_entries (user_id, vault_id, entry_type, amount, status) VALUES ($1, $2, 'DEPOSIT_BUYBACK', $3, 'SWEPT')`, [userId, targetVaultId, userBuybackAmount]);
                ...
                const buybackDesc = `Received ${userBuybackAmount.toFixed(2)} USDC from Bonus Point buyback via ${overallDescription}.`;
                ...
                await awardXp({
                    userId: userId, xpAmount: userBuybackAmount * 0.1, type: 'BONUS_POINT_BUYBACK_XP',
            ...
            if (userXpAmount > 0.000001) {
                ...
                const xpDesc = `Received ${userXpAmount.toFixed(2)} USDC from XP-weighted rewards via ${overallDescription}.`;
        ...
        const treasuryDesc = `Executed Hybrid Distribution: "${overallDescription}". Total: $${numericReward.toFixed(2)}. Admin: ${adminUserId}.`;
