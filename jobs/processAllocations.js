...
async function processAllocations() {
  ...
  try {
    ...
    // Process one at a time to avoid overwhelming the hot wallet or hitting rate limits
    for (const position of positionsToProcess) {
      ...
      try {
        ...
        // --- Step B: Verify Gas Arrival (Wait up to 60s) ---
        let gasBalanceConfirmed = false;
        for (let i = 0; i < 12; i++) { // 12 retries * 5 seconds = 60 seconds
          const currentBalance = await provider.getBalance(eth_address);
          if (currentBalance.gte(fundingAmount_BN)) {
            console.log(`✅ Gas balance confirmed on-chain: ${ethers.utils.formatEther(currentBalance)} ETH`);
        ...
        if (!gasBalanceConfirmed) {
          throw new Error("Gas funding did not appear on-chain within 60 seconds.");
        }

        // --- Step C: The Split & Sweep (Now Sequential) ---
        ...
        console.log(`✅ Trading Desk sweep confirmed. TX: ${tradeDeskTx.hash}`);
        ...
        console.log(`✅ Devops sweep confirmed. TX: ${devopsTx.hash}`);
        ...
        await client.query(`UPDATE user_vault_positions SET status = 'sweep_failed' WHERE position_id = $1`, [position_id]);
      }
    }
  } catch (err) {
