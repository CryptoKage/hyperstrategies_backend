...
const processTimeWeightedRewards = async () => {
  ...
  try {
    // --- ANNOTATION: The query now looks for 'in_trade' OR 'active' positions. ---
    // This makes the job more robust if the sweep job is delayed.
    ...
    for (const position of positions) {
      ...
      try {
        ...
        console.error(`Error processing position for user ${position.user_id}. Rolling back transaction.`, innerErr);
      }
    }
