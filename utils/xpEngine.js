...
async function awardXp(options, dbClient) {
  const { userId, xpAmount, type, descriptionKey, descriptionVars } = options;
  ...
  // We now store the key and the variables as a JSON string in the 'description' column.
  const descriptionPayload = JSON.stringify({
    key: descriptionKey,
    vars: descriptionVars || {}
  });
  ...
  await dbClient.query(
    ...
    [userId, `XP_${type}`, type, descriptionPayload, finalXpAmount]
  );
}
